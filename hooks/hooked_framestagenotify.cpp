...
void __stdcall hooked_framestagenotify(ClientFrameStage_t stage)
{
	...
	if (!engine->IsInGame())
	{
		if (was_in_game)
		{
			...
			visuals->player_model_indexes.clear();
		...
		if (ctx->local()->valid())
		{
			...
			if (config->visuals.world.client_bullet_impacts)
			{
				...
				for (auto i = client_impact_list.Count(); i > last_count; --i)
					debugoverlay->BoxOverlay(client_impact_list[i - 1].position, Vector(-1.0f, -1.0f, -1.0f), Vector(1.0f, 1.0f, 1.0f), QAngle(0.0f, 0.0f, 0.0f), (int)(config->visuals.world.client_bullet_impacts_clr[0] * 255.0f), (int)(config->visuals.world.client_bullet_impacts_clr[1] * 255.0f), (int)(config->visuals.world.client_bullet_impacts_clr[2] * 255.0f), (int)(config->visuals.world.client_bullet_impacts_clr[3] * 255.0f), 4.0f);
