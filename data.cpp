...
void signatures_initialize()
{
	...
	signatures_manager->signatures[SIGNATURE_DRAW_MODEL_STATS_OVERLAY] = signatures_manager->find_signature(crypt_str("client.dll"), crypt_str("8B 0D ?? ?? ?? ?? 8B D0 85 D2 0F 85 A0"));
	...
	signatures_manager->signatures[SIGNATURE_CLEAR_CUSTOM_MATERIALS] = signatures_manager->follow_rel32(signatures_manager->find_signature(crypt_str("client.dll"), crypt_str(" E8 ? ? ? ? 51 6A 26 ")),1);
