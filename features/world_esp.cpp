...
void WorldEsp::weaponEsp()
{
	...
	{
		Color clr_temp = Color(config->visuals.world.weapon_type[WEAPON].box_clr);
	...
	if (config->visuals.world.weapon_type[WEAPON].ammo && !(client_class->m_ClassID == CBaseGrenade || client_class->m_ClassID == CC4 ||
		...
		if (config->visuals.world.weapon_type[WEAPON].ammo_type[0]) {
			...
			if (h) {
				...
				Color clr_temp = Color(config->visuals.world.weapon_type[WEAPON].ammo_clr);
	...
	Color clr_temp = Color(config->visuals.world.weapon_type[WEAPON].ammo_clr);
	Color clr_icon = Color(config->visuals.world.weapon_type[WEAPON].icon_clr);
...
void WorldEsp::c4Esp()
{
	...
	{
		Color clr_temp = Color(config->visuals.world.weapon_type[C4].box_clr);
	...
	if (config->visuals.world.weapon_type[C4].icon)
		renderer_imgui.RenderText(Get_UTF8(buf), ImVec2((box.x + (box.w - box.x) / 2), box.y + u::dpi(14.0f)), u::dpi(11.25f), Color(config->visuals.world.weapon_type[C4].icon_clr), true, core::f::icons_weapon, true,true);
...
void WorldEsp::run()
{
	...
	for (int i = 1; i <= entitylist->GetHighestEntityIndex(); i++)  //-V807
	{
		...
		if (!skip_weapon_esp)
		{
			if (config->visuals.world.weapon_type[WEAPON].esp && (client_class->m_ClassID == CAK47 || client_class->m_ClassID == CDEagle || client_class->m_ClassID == CBaseGrenade || client_class->m_ClassID == CC4 ||
				client_class->m_ClassID == CSensorGrenade || client_class->m_ClassID == CSmokeGrenade || client_class->m_ClassID == CDecoyGrenade ||
				client_class->m_ClassID == CHEGrenade || client_class->m_ClassID == CIncendiaryGrenade || client_class->m_ClassID == CMolotovGrenade ||
			...
			else if (config->visuals.world.weapon_type[C4].esp && client_class->m_ClassID == CPlantedC4)
			{
				m_origin = m_e->GetAbsOrigin();
				if (getBox())
					c4Esp();
