...
void SoundEsp::run()
{
	...
	for (auto i = 0; i < m_cursoundlist.Count(); i++)
	{
		...
		{
		case ENEMY:
			sounds_radius = config->visuals.player_type[ENEMY].sounds_radius;
			break;
		case LOCAL:
			sounds_radius = config->visuals.player_type[LOCAL].sounds_radius;
			break;
		case TEAM:
			sounds_radius = config->visuals.player_type[TEAM].sounds_radius;
			break;
		}
...
void SoundEsp::draw()
{
	...
	for (auto i = 0; i < buffer.size(); ++i)
	{
		...
		if (circle.player_is == ENEMY && !config->visuals.player_type[ENEMY].sounds || circle.player_is == LOCAL && !config->visuals.player_type[LOCAL].sounds || circle.player_is == TEAM && !config->visuals.player_type[TEAM].sounds)
			continue;
		...
		{
		case ENEMY:
			sounds_time = config->visuals.player_type[ENEMY].sounds_time;
			sounds_color[0] = config->visuals.player_type[ENEMY].sounds_color[0];
			sounds_color[1] = config->visuals.player_type[ENEMY].sounds_color[1];
			sounds_color[2] = config->visuals.player_type[ENEMY].sounds_color[2];
			sounds_color[3] = config->visuals.player_type[ENEMY].sounds_color[3];
			break;
		case LOCAL:
			sounds_time = config->visuals.player_type[LOCAL].sounds_time;
			sounds_color[0] = config->visuals.player_type[LOCAL].sounds_color[0];
			sounds_color[1] = config->visuals.player_type[LOCAL].sounds_color[1];
			sounds_color[2] = config->visuals.player_type[LOCAL].sounds_color[2];
			sounds_color[3] = config->visuals.player_type[LOCAL].sounds_color[3];
			break;
		case TEAM:
			sounds_time = config->visuals.player_type[TEAM].sounds_time;
			sounds_color[0] = config->visuals.player_type[TEAM].sounds_color[0];
			sounds_color[1] = config->visuals.player_type[TEAM].sounds_color[1];
			sounds_color[2] = config->visuals.player_type[TEAM].sounds_color[2];
			sounds_color[3] = config->visuals.player_type[TEAM].sounds_color[3];
			break;
		}
