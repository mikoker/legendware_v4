...
void LocalAnimations::render()
{
	...
	if (update_fake_bones || !config->visuals.player_type[LOCAL].visualize_lag)
	{
...
void LocalAnimations::run(crypt_ptr <CUserCmd> cmd, bool send_packet) //-V813
{
	...
	if (config->anti_aim.break_local_animation[STATIC_AIR])
		ctx->local()->m_flPoseParameters()[6] = 1;

	if (config->anti_aim.break_local_animation[REVERSE_LEGS])
	{
		if (globals->tickcount % 4 > 1)
			ctx->local()->m_flPoseParameters()[0] = 1;
	}

	if (config->anti_aim.break_local_animation[ALLAH_LEGS] & 4)
		ctx->local()->m_flPoseParameters()[7] = 1;
