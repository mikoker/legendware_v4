	...
	for (auto i = 1; i <= globals->maxclients; ++i) //-V688
	{
		...
		if (player_is == ENEMY) {
			for (auto i = 0; i < FLAGS_MAX; ++i) //-V688
				flags_current[i] = config->visuals.player_type[ENEMY].esp_flags_arr[i];

			for (auto i = 0; i < 2; ++i) //-V688
				esp_weapon_arr_current[i] = config->visuals.player_type[ENEMY].esp_weapon_arr[i];
			...
			if (config->visuals.player_type[ENEMY].esp_weapon)
				weapon();
			if (config->visuals.player_type[ENEMY].esp_skeleton)
				draw_skeleton();
			if (config->visuals.player_type[ENEMY].esp_ammo)
				ammo();

#ifdef BETA
			if (!animations->animation_data[i].empty() && config->visuals.player_type[ENEMY].multipoints)
			{
		...
		else if (player_is == LOCAL)
		{
			...
			if (!input->m_fCameraInThirdPerson)
				continue;
			for (auto i = 0; i < FLAGS_MAX; ++i) //-V688
				flags_current[i] = config->visuals.player_type[LOCAL].esp_flags_arr[i];

			for (auto i = 0; i < 2; ++i) //-V688
				esp_weapon_arr_current[i] = config->visuals.player_type[LOCAL].esp_weapon_arr[i];
			...
			if (config->visuals.player_type[LOCAL].esp_weapon)
				weapon();
			if (config->visuals.player_type[LOCAL].esp_skeleton)
				draw_skeleton();
		...
		{
			for (auto i = 0; i < FLAGS_MAX; ++i) //-V688
				flags_current[i] = config->visuals.player_type[TEAM].esp_flags_arr[i];

			for (auto i = 0; i < 2; ++i) //-V688
				esp_weapon_arr_current[i] = config->visuals.player_type[TEAM].esp_weapon_arr[i];
			...
			if (config->visuals.player_type[TEAM].esp_weapon)
				weapon();
			if (config->visuals.player_type[TEAM].esp_skeleton)
				draw_skeleton();
...
void Esp::ammo() {
	...
	else
	{
		if (player_is == ENEMY)
		{
			temp = Color(config->visuals.player_type[ENEMY].esp_ammo_clr);
		}
		else if (player_is == LOCAL)
		{
			temp = Color(config->visuals.player_type[LOCAL].esp_ammo_clr);
		}
		else if (player_is == TEAM)
		{
			temp = Color(config->visuals.player_type[TEAM].esp_ammo_clr);
		}
	}
...
void Esp::draw_skeleton()
{
	...
	if (player_is == ENEMY)
	{
		clr = Color(config->visuals.player_type[ENEMY].esp_skeleton_clr);
	}
	else if (player_is == LOCAL) {
		clr = Color(config->visuals.player_type[LOCAL].esp_skeleton_clr);
	}
	else if (player_is == TEAM)
	{
		clr = Color(config->visuals.player_type[TEAM].esp_skeleton_clr);
	}
...
void Esp::weapon()
{
	...
	if (player_is == ENEMY)
	{
		clr = weapon_data->weapon_type == WEAPONTYPE_KNIFE_TASER ? Color(config->visuals.player_type[ENEMY].esp_knife_taser_clr) : Color(config->visuals.player_type[ENEMY].esp_weapon_clr);
	}
	else if (player_is == LOCAL) {
		clr = weapon_data->weapon_type == WEAPONTYPE_KNIFE_TASER ? Color(config->visuals.player_type[LOCAL].esp_knife_taser_clr) : Color(config->visuals.player_type[LOCAL].esp_weapon_clr);
	}
	else if (player_is == TEAM)
	{
		clr = weapon_data->weapon_type == WEAPONTYPE_KNIFE_TASER ? Color(config->visuals.player_type[TEAM].esp_knife_taser_clr) : Color(config->visuals.player_type[TEAM].esp_weapon_clr);
	}
...
void Esp::box()
{
	...
	if (player_is == ENEMY)
	{
		clr = Color(config->visuals.player_type[ENEMY].esp_box_clr);
	}
	else if (player_is == LOCAL) {
		clr = Color(config->visuals.player_type[LOCAL].esp_box_clr);
	}
	else if (player_is == TEAM)
	{
		clr = Color(config->visuals.player_type[TEAM].esp_box_clr);
	}
...
void Esp::name()
{

	{
		...
		if (player_is == ENEMY)
		{
			clr = Color(config->visuals.player_type[ENEMY].esp_name_clr);
		}
		else if (player_is == LOCAL)
		{
			clr = Color(config->visuals.player_type[LOCAL].esp_name_clr);
		}
		else if (player_is == TEAM)
		{
			clr = Color(config->visuals.player_type[TEAM].esp_name_clr);
		}
...
{
	{
		...
		if ((player_is == ENEMY && config->visuals.player_type[ENEMY].esp_hp_bar_custom_color) || (player_is == LOCAL && config->visuals.player_type[LOCAL].esp_hp_bar_custom_color) || (player_is == TEAM && config->visuals.player_type[TEAM].esp_hp_bar_custom_color)) {
			Color temp;
			...
			else
			{
				if (player_is == ENEMY)
				{
					temp = Color(config->visuals.player_type[ENEMY].esp_hp_bar_color);
				}
				else if (player_is == LOCAL) {
					temp = Color(config->visuals.player_type[LOCAL].esp_hp_bar_color);
				}
				else if (player_is == TEAM)
				{
					temp = Color(config->visuals.player_type[TEAM].esp_hp_bar_color);
				}
...
Color Esp::alpha_flags_set(int FLAG)
{
	...
	if (!player->IsDormant()) {
		...
		if (player_is == ENEMY)
		{
			clr_temp = Color(config->visuals.player_type[ENEMY].esp_flags_combo_clr[FLAG]);
		}
		else if (player_is == LOCAL) {
			clr_temp = Color(config->visuals.player_type[LOCAL].esp_flags_combo_clr[FLAG]);
		}
		else if (player_is == TEAM)
		{
			clr_temp = Color(config->visuals.player_type[TEAM].esp_flags_combo_clr[FLAG]);
		}
		clr = clr_temp;
