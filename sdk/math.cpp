...
namespace math
{
	...
	float normalize_angles(const float angle)
	{
		float normalized_angle = fmod(angle, 360.f);

		if (normalized_angle > 180.f)
	...
	{
		return std::remainder(std::remainder(dst, 360.f) - std::remainder(src, 360.f), 360.f);
	}
}