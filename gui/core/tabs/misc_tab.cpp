...
namespace core::tabs::misc
{
	void first_tab()
	{
		...
		imgui_ex::mcombo(crypt_str("Bypasses"), config->misc.bypasses.data(), v::bypasses, IM_ARRAYSIZE(v::bypasses));
		...
		imgui_ex::mcombo_clr(crypt_str("Logs"), config->misc.logs.data(), config->misc.logs_color, v::logs, IM_ARRAYSIZE(v::logs));

		if (config->misc.logs[LOGS_HITS] || config->misc.logs[LOGS_MISSES] || config->misc.logs[LOGS_HURTS] || config->misc.logs[LOGS_PURCHASES])
			imgui_ex::mcombo(crypt_str("Position"), config->misc.logs_position.data(), v::logs_position, IM_ARRAYSIZE(v::logs_position));
		...
		{
			imgui_ex::mcombo(crypt_str("Modes"), config->misc.automatic_peek.data(), v::automatic_peek_mode, IM_ARRAYSIZE(v::automatic_peek_mode));
	...
	void third_tab()
	{
		...
		if (config->misc.automatic_buy)
		{
			...
			imgui_ex::mcombo(crypt_str("Others"), config->misc.others.data(), others, IM_ARRAYSIZE(others));
		}
