...
namespace imgui_ex
{
	...
	bool mcombo(const char* label, int* v, const char* items[], int items_count)
	{
		const bool has_shit = strstr(label, "##") != nullptr;
		...
		if (ImGui::BeginCombo(label, preview.c_str(), 0, &expected_h))
		{
			for (int i = 0; i < items_count; i++)
			{
				if (selectable(items[i], (bool*)v[i], ImGuiSelectableFlags_DontClosePopups, ImVec2(0.0f, 0.0f), expected_h, i , (bool*)&v[i]))
				{
					if (v[i] == 1)
						v[i] = 0;
					else
	...
	bool mcombo_clr(const char* label, int* v,
		float clr[][4], const char* items[], int items_count, float width)
	{
			...
			ImGuiComboFlags_ColorPicker, &expected_h, &close))
		{
			for (int i = 0; i < items_count; i++)
			{
				if (selectable_color(items[i], (bool*)v[i], clr[i],
					ImGuiSelectableFlags_DontClosePopups, ImVec2(0.0f, 0.0f), expected_h, close,i, (bool*)&v[i]))
				{
					if (v[i] == 1)
						v[i] = 0;
					else
