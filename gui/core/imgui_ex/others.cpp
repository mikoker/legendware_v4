...
#define IMGUI_P ImVec2(0.0f, 0.0f), ImVec2(1.0f, 1.0f)
	...
	void hitboxes(const char* label, const ImVec2& size_arg, int& head_pr,
		int& body_pr, int& pelvis_pr, int& legs_pr, int& feet_pr, int& arms_pr)
	{
		ImGuiWindow* window = ImGui::GetCurrentWindow();
		...
		for (size_t i = 0; i < bb_hitboxes.size(); i++)
		{
			...
			if (hovered)
			{
				if (ImGui::IsMouseClicked(ImGuiMouseButton_Left))
				{
					switch (i)
					{
					case 0: head_pr = min(1, head_pr + 1); break;
					case 1: body_pr = min(1, body_pr + 1); break;
					case 2: pelvis_pr = min(1, pelvis_pr + 1); break;
					case 3: legs_pr = min(1, legs_pr + 1); break;
					case 4: feet_pr = min(1, feet_pr + 1); break;
					case 5: case 6: arms_pr = min(1, arms_pr + 1); break;
					default: break;
					}
				}
				...
				if (ImGui::IsMouseClicked(ImGuiMouseButton_Right))
				{
					switch (i)
					{
					case 0: head_pr = max(0, head_pr - 1); break;
					case 1: body_pr = max(0, body_pr - 1); break;
					case 2: pelvis_pr = max(0, pelvis_pr - 1); break;
					case 3: legs_pr = max(0, legs_pr - 1); break;
					case 4: feet_pr = max(0, feet_pr - 1); break;
					case 5: case 6: arms_pr = max(0, arms_pr - 1); break;
					default: break;
					}
				}
			}
...
#define IMGUI_P ImVec2(0.0f, 0.0f), ImVec2(1.0f, 1.0f)
		...
		const ImVec2& size_arg)
	{
		...
		if ((key_bind.key > KEY_NONE && key_bind.key < KEY_MAX) && g.ActiveId != id)
		{
